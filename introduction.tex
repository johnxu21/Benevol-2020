...
For the technical aspects, ecosystems are often characterised by numerous dependency relationships between their on the packages on software package distribution platforms~\cite{decan:2016:ECSAW}. To this end, we would to investigate if developers migrate from one variant of the required package to another in a family.
...
Before we can proceed in stating the research questions of our study let us first discuss the terminologies we shall use in our analysis: 
...
To put the the presented terminologies in perspective, let us present a concrete example of a software family. The mainline repository \texttt{creationix/wheat} is hosted on \gh (with a total of 136 forks), is a blog engine for coders written in \texttt{node.JS}. The mainline and 2 of 136 forks (\texttt{sun11/wheat} and \texttt{frodare/barley}) have their package releases distributed on \npm. In Table~\ref{tab:example} we present statistics corresponding to the package releases, package dependencies, dependent packages and dependent projects for three repositories in the software family.

%The following are the numbers of package releases, package dependencies, dependents packages and dependents projects for the mainline and its two variants:
...
As stated earlier in this section, since this research the first of its kind, we would like to state some exploratory questions as a preliminary step of investigates the socio-technical aspects of forking in the \js ecosystem. 
Below are the four research questions for our study
\begin{itemize}
\item[\textbf{$RQ_0$}] \textit{How prevalent are software families in \np?}
...
\item[\textbf{$RQ_1$}] \textit{How do the distributions of package releases in mainlines and their variants compare to each other? }\tm{It is still not clear to me, from reading the text, what you mean by "package releases in mainlines". How is a package release linked to a mainline repository and to variant (forked) repositories? Perhaps you should provide some concrete example somewhere (probably even before starting the RQs) to make all of this clear to the reader (including the link between packages, package releases, package dependencies, github repositories and their forks.}
This RQ will help us determine if mainlines and variants are continuously maintained.\tm{Why is it important or relevant to know if they are continuously maintained?}
...
This RQ will help us determine if members in the software families depend on other packages in the \np ecosystem.\tm{Unclear what is meant by "members in the software families depend on other packages}

\item[\textbf{$RQ_3$}] \textit{Do the variant projects have dependent packages\,/\,projects?}
