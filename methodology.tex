...
\textbf{Dataset}. Our dataset\tom{Dataset of what? Of npm packages? Of \gh repositories? Of dependencies?} was extracted from the 2019-02-02 \note{Which dataset is this? I couldn't find it in libraries.io} dump of the open source discovery service \texttt{libraries.io}\cite{libraries.io}.
\tom{I agree with this comment. The most recent datadumps that are listed are 1.6.0 on January 12, 2020 and 1.4.0 on December 22, 2018 (see https://libraries.io/data). Both of them are available for download from Zenodo, but nothing for 1.5 though, and nothing for 2019 either!}
On \texttt{libraries.io}, we use the following heuristics to extract the software families: The repository $A$ is a variant of the repository $B$ if (1) $A$ and $B$ are distributed on \npm,\tom{"repositories" are not distributed on npm, only package releases are. Moreover, there is not necessarily a one-to-one mapping between repositories and package releases} and (2) $A$ is a fork--of $B$ on \scp.\tom{On \gh?} Since we focus on the \js ecosystem on \gh, we extracted software families whose repositories are hosted on \gh and their packages distributed on the \np package distribution platform.\tom{This sentence comes much too late and is not sufficiently detailed.} For each \textbf{mainline} and its corresponding \textbf{variants} we collected their \textbf{releases}, \textbf{dependencies} and \textbf{dependents} (projects and packages).\tom{A concrete example is at its place here.}
